...
async function loadAllFunnels(): Promise<void> {
  isLoading.value = true
  loadError.value = null
  try {
    const api = useApi()
    const response = await api<AllWorkspacesFunnelList>('/funnels')
...
const filteredFunnels = computed<FunnelWithWorkspace[]>(() => {
  if (!searchQuery.value.trim()) return funnels.value
  const q = searchQuery.value.toLowerCase()
  return funnels.value.filter(
    f =>
      f.name.toLowerCase().includes(q)
      || f.workspace.name.toLowerCase().includes(q),
  )
})

// ---------------------------------------------------------------------------
// Funnels nach Workspace gruppieren
// ---------------------------------------------------------------------------
...
const groupedByWorkspace = computed<WorkspaceGroup[]>(() => {
  ...
  for (const funnel of filteredFunnels.value) {
...
<template>
  <div class="mx-auto max-w-5xl p-6">
    ...
    <div
      v-else-if="!isLoading && groupedByWorkspace.length > 0"
      class="space-y-8"
    >
      <section
